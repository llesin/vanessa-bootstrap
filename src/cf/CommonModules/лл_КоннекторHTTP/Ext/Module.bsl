...
#Область Служебные_процедуры_и_функции_Переопределяемый

Функция ПолучитьДанные1СИзСтрокиBase64(Знач СтрокаBase64) Экспорт

	Данные1С = Неопределено;
	Если НЕ ТипЗнч(СтрокаBase64) = Тип("Строка") Тогда
		ВызватьИсключение "Входящие данные должны иметь тип ""Строка"". Тип входящих данных: " + Строка(ТипЗнч(СтрокаBase64));
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(СтрокаBase64) Тогда
		ВызватьИсключение "Входящие данные не заполнены!";
	КонецЕсли;
	
	Попытка
		СтрокаBase64 = СтрЗаменить(СтрокаBase64, Символ(182), "");
		
		ДД_Данных1С 	= Base64Значение(СтрокаBase64);
		СтрокаJSON 		= ПолучитьСтрокуИзДвоичныхДанных(ДД_Данных1С);                             
		
		ЧтениеJson = Новый ЧтениеJSON;                       
		ЧтениеJson.УстановитьСтроку(СтрЗаменить(СтрокаJSON, Символы.ПС, ""));                    
		Данны1С = СериализаторXDTO.ПрочитатьJSON(ЧтениеJson);
	Исключение
		СообщениеОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение "Не удалось десериализовать строку base64: " + Символы.ПС + СообщениеОбОшибке;
	КонецПопытки;
...
Функция ПолучитьBase64СтрокуИзДанных1С(Данные1С) Экспорт
	
	вхДанные_Base64 = "";
	
	Поток = Новый ПотокВПамяти();	
	ЗаписьJson = Новый ЗаписьJson; 
	ЗаписьJSON.ОткрытьПоток(Поток, КодировкаТекста.UTF8,, Новый ПараметрыЗаписиJson());
	СериализаторXDTO.ЗаписатьJSON(ЗаписьJson, Данные1С, НазначениеТипаXML.Явное); 
	ЗаписьJson.Закрыть();
	
	ДД_вхДанных = Поток.ЗакрытьИПолучитьДвоичныеДанные();
	Если НЕ ДД_вхДанных = Неопределено Тогда
		вхДанные_Base64 = ПолучитьBase64СтрокуИзДвоичныхДанных(ДД_вхДанных);
	КонецЕсли;	
	
	Возврат вхДанные_Base64;
	
КонецФункции

Процедура ДобавитьСообщениеОбОшибкеВЛог(ЛогОшибок, СообщениеОбОшибке, ЗаголовокСообщения = "") Экспорт

	Если НЕ ЛогОшибок = "" Тогда
		ЛогОшибок = ЛогОшибок + Символы.ПС;
		
	КонецЕсли;
	
	Если ЗаголовокСообщения = "" Тогда
		ЛогОшибок = ЛогОшибок + СообщениеОбОшибке;
		Возврат;
	КонецЕсли;
	
	// С заголовком сносим подчиненные вправо
	ЛогОшибок = ЛогОшибок + Символы.ПС + ЗаголовокСообщения + Символы.ПС;
	мСообщений = СтрРазделить(СообщениеОбОшибке, Символы.ПС);
	Для Каждого Сообщение Из мСообщений Цикл
		ЛогОшибок = ЛогОшибок + Символы.ПС + Символы.Таб + Сообщение;
	КонецЦикла;
